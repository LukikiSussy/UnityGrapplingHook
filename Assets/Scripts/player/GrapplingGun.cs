...
public class GrapplingGun : MonoBehaviour
{
    ...
    Vector3 mousePos;
    Vector3 currentPos;
    [HideInInspector] public Vector3 distanceNorm;
    ...
    void CalculateDistance() {
        ...
        Vector2 distance = mousePos - new Vector2(grapplingGunPivot.position.x, grapplingGunPivot.position.y);

        distanceNorm = distance.normalized;

        if (springJoint != null) {

            distanceToGrapplePoint = (grapplePoint - grapplingGunPivot.transform.position).normalized;
